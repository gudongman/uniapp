<template>
    <view class="conbox">
        <view class="container">
            <!-- 背景 -->
            <image src="@/static/images/luckDraw/bg.png" class="cont" mode=""></image>
            <image src="@/static/images/luckDraw/caidai.png" class="caidai" mode=""></image>
    ...
    methods: {
	  ...
	  random(){
		  ...
		  let sum = this.list.reduce((pre, item)=>{
			  return item.chance + pre
		  },0)
		  console.log('权重值总和' , sum);
		  let num = (Math.random() *sum ).toFixed(2);
		  console.log(num);
		  // 找到最接近的值 为中奖的号码
		  ...
		  console.log(arr);
		  let randomIndex = arr.findIndex((item,index)=>{
			  return item.value == 'random'
		  })
		  randomIndex = Math.min(randomIndex, this.list.length-1)
		 // randomIndex 取this.list[randomIndex] 为抽中的奖项
		 return randomIndex;
      ...
      playReward()
      {
        ...
        setTimeout(() => {
          uni.showModal({content: that.list[index].isNoPrize ? '抱歉，您未中奖' : '恭喜，中奖'})
          this.btnDisabled = '';
